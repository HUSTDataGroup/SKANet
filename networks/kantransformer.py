...
class KAN(nn.Module):
    def __init__(
        ...
        self.gamma = nn.Parameter(torch.ones(hidden_features)) 
        self.p_logit1 = nn.Parameter(torch.tensor([0.0])) 
        self.p_logit2 = nn.Parameter(torch.tensor([0.0]))

    def forward(self, x):
        x = self.act1(x)
        p_d1 = torch.sigmoid(self.p_logit1)
        x = F.dropout(x, p=p_d1.item(), training=self.training)
        x = self.fc1(x)
        x = self.act2(x)
        if x.dim() == 4:
            ...
            x = x * self.gamma
        p_d2 = torch.sigmoid(self.p_logit2)
        x = F.dropout(x, p=p_d2.item(), training=self.training)
        x = self.fc2(x)
        return x
