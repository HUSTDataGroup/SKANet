...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        self.dgrkan = DGRKAN(mid_channels, use_conv=True, act_init='gelu')
        self.conv2 = nn.Conv2d(mid_channels, out_channels, kernel_size=3, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(out_channels)
        if skip_channels != 0:
            self.mscam = MSKAM(in_channels=skip_channels, out_channels=skip_channels)

    def forward(self, x, skip=None):
        x = self.up(x)
        if skip is not None:
            skip = self.mscam(skip)
